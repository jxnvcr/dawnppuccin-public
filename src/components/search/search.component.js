class Search extends Component {
    ...
    style() {
      return `
        ...
        #search input::selection {
            background:rgba(212, 117, 215, 0.54);
            color: #fff;
        }
    ...
    open(input) {
      window.open(input, "_" + CONFIG.tabOpenMode, "noopener noreferrer");
    }
    ...
    async handleSearch(event) {
      ...
      let prefix = args[0];
      let defaultEngine = this.engines['default'][0];
      ...
      if (key === 'Enter') {
        ...
        if (this.isValidUrl(args.join(' '))) {
          this.open(args.join(' ').includes('://') ? args.join(' ') : `https://${args.join(' ')}`)
        }
        else {  
          const links = CONFIG.tabs.flatMap(
            tab => tab.categories.flatMap(
              category => category.links.map(
                link => ({name: link.name.toLowerCase(), alt: link.alt, url: link.url})
              )
            ));
            
          const existedLink = links.find(link => link.name === args.join(' ').toLowerCase() || link.alt.includes(args.join(' ')));
            
          if (existedLink) {
            this.open(existedLink.url);
          }
          else {
            this.open(engine + encodeURI(args.join(' ')));
          }
        }
      }
